                                ...
                                return (
                                  <div key={idx} className={`rounded-xl border p-3 space-y-1.5 transition-all ${c.card}`}>
                                    <div className="flex items-center justify-between gap-2">
                                      <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded uppercase tracking-wider ${c.badge}`}>
                                        #{idx + 1} — {label}
                                      </span>
                                      {!rev.isApproved && (
                                        <button
                                          ...
                                          onClick={async () => {
                                            try {
                                              ...
                                              toast.success(`Đã duyệt gợi ý #${idx + 1} thành công.`);
                                              loadReviews();
                                            } catch (err: any) {
                                              toast.error('Lỗi khi duyệt: ' + err.message);
                                            }
                                          }}
                                          className={`flex items-center gap-1 px-2.5 py-1 text-white rounded-lg text-[10px] font-bold shadow-sm cursor-pointer shrink-0 ${c.btn}`}
                                        >
                                    ...
                                    <p className="text-xs text-slate-800 leading-relaxed italic">🇻🇳 {viText}</p>
                                    {enText && <p className="text-[11px] text-slate-500 leading-relaxed italic">🇬🇧 {enText}</p>}
                                  </div>
                                );
                              })}
                            </div>
